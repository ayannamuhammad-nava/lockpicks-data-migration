...
class SchemaGenerator:
    """Generates normalized schema from enriched legacy metadata.

    Supports multiple target platforms (PostgreSQL, Snowflake, Oracle,
    AWS Redshift) via pluggable target adapters.
    """
    ...
    def _map_base_type(self, legacy_type: str) -> str:
        """Map a legacy type to the target platform, preserving size qualifiers.

        Delegates to the target adapter when one is set; falls back to the
        built-in TYPE_MAP (PostgreSQL) for backward compatibility.
        """
        if self._target:
            return self._target.map_type(legacy_type)
    ...
    def render_ddl(self, table: GeneratedTable) -> str:
        ...
        Delegates to the target adapter when one is set; falls back to
        built-in PostgreSQL DDL rendering for backward compatibility.
        """
        if self._target:
            col_dicts = [
                {
            ...
            return self._target.render_create_table(
        ...
        # Fallback: built-in PostgreSQL rendering
        lines = []
    ...
    def render_full_ddl(
        ...
        target_name = self._target.dialect_name() if self._target else "postgres"
        from dm.targets.postgres import TARGET_DISPLAY_NAMES
        display_name = TARGET_DISPLAY_NAMES.get(target_name, target_name.title())

        lines = []
        lines.append("-- ============================================================")
        lines.append(f"-- DM Generated Schema — {display_name}")
    ...
    def render_transforms(self, source_table: str, target: GeneratedTable) -> str:
        ...
        Uses target adapter function translation when available so that
        hash, timestamp, and cast expressions use the correct dialect.
        """
        dialect = self._target.dialect_name() if self._target else "postgres"

        lines = []
        lines.append(f"-- Transform script: {source_table} → {target.name}")
        lines.append(f"-- Target: {dialect}")
        ...
        for col in target.columns:
            if not col.source_column:
                if col.name in ("created_at", "updated_at"):
                    ...
                    now_expr = self._target.translate_function("now", []) if self._target else "NOW()"
            ...
            if col.transform == "SHA-256":
                select_exprs.append(self._render_hash_expr(col.source_column, dialect))
            elif col.data_type in ("BOOLEAN", "NUMBER(1)"):
                select_exprs.append(
                    f"CASE WHEN UPPER({col.source_column}) IN ('Y','YES','T','TRUE','1') "
                    f"THEN {self._render_true_literal(dialect)} "
                    f"ELSE {self._render_false_literal(dialect)} END"
                )
            elif col.data_type == "DATE" and col.source_column:
                select_exprs.append(self._render_cast_date(col.source_column, dialect))
            else:
    ...
    def _render_hash_expr(self, source_col: str, dialect: str) -> str:
        """Render a SHA-256 hash expression in the target dialect."""
        if dialect == "snowflake":
            return f"SHA2({source_col}::VARCHAR, 256)"
        elif dialect == "oracle":
            return f"LOWER(RAWTOHEX(DBMS_CRYPTO.HASH(UTL_RAW.CAST_TO_RAW({source_col}), 4)))"
        elif dialect == "redshift":
            return f"SHA2({source_col}::VARCHAR, 256)"
        else:
            return f"encode(sha256({source_col}::bytea), 'hex')"

    def _render_cast_date(self, source_col: str, dialect: str) -> str:
        """Render a CAST to DATE in the target dialect."""
        if dialect == "oracle":
            return f"TRUNC({source_col})"
        elif dialect in ("snowflake", "redshift"):
            return f"{source_col}::DATE"
        else:
            return f"{source_col}::DATE"

    def _render_true_literal(self, dialect: str) -> str:
        if dialect == "oracle":
            return "1"
        return "TRUE"

    def _render_false_literal(self, dialect: str) -> str:
        if dialect == "oracle":
            return "0"
        return "FALSE"
    ...
    def save_artifacts(self, result: SchemaGenResult, output_path: Path) -> None:
        ...
        Saves the default DDL (current target or PostgreSQL fallback) to
        output_path, plus metadata files (diff report, datasets, mappings).
        """
    ...
    def save_all_targets(
        ...
        glossary: Dict,
        mappings: Dict,
        om_profiles: Optional[Dict],
        output_base: Path,
    ) -> Dict[str, SchemaGenResult]:
        """Generate and save DDL for all available target platforms.

        Creates a subfolder per target under output_base:
            output_base/postgres/
            output_base/snowflake/
            output_base/oracle/
            output_base/redshift/

        Returns:
            Dict of {target_name: SchemaGenResult}.
        """
        from dm.targets.postgres import get_available_targets, get_target_adapter

        results = {}
        for target_key in get_available_targets():
            adapter = get_target_adapter(target_key)
            target_gen = SchemaGenerator(
                config=self._config,
                plugin_manager=self._pm,
                om_enricher=self._om,
                target_adapter=adapter,
            )
            result = target_gen.generate(
            ...
            target_dir = output_base / target_key
            target_gen.save_artifacts(result, target_dir)
            results[target_key] = result
            logger.info(f"Generated {target_key} schema in {target_dir}")
